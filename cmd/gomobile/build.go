...
func runBuild(cmd *command) (err error) {
	...
	if !nmpkgs["github.com/thommil/tge-mobile/app"] {
		return fmt.Errorf(`%s does not import "github.com/thommil/tge-mobile/app"`, pkg.ImportPath)
	}

	return nil
}

var nmRE = regexp.MustCompile(`[0-9a-f]{8} t (?:.*/vendor/)?(github.com/thommil.*/[^.]*)`)

func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/thommil/tge-mobile/app": true}, nil
	}
	r, w := io.Pipe()
...
func importsApp(pkg *build.Package) error {
	...
	for _, path := range pkg.Imports {
		if path == "github.com/thommil/tge-mobile/app" {
			return nil
		}
	}
	return fmt.Errorf(`%s does not import "github.com/thommil/tge-mobile/app"`, pkg.ImportPath)
